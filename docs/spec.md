# ccplan — Claude Code Plan CLI ツール 要件定義書

## 概要

Claude Codeのplanモードで生成されるプランファイルを操作するためのCLIツール。
プランの可視化・レビュー・検出・Claude Code hookとの統合を1バイナリで提供する。

### コマンド名: `ccplan`

> "cc" = Claude Code。`plan` 単体だと汎用的すぎるため。

### サブコマンド一覧

| サブコマンド | 説明 | フェーズ |
|-------------|------|---------|
| `ccplan review` | プランをTUIで可視化し、インラインレビューコメントを付与 | Phase 1 |
| `ccplan locate` | transcript/設定からプランファイルのパスを特定して出力 | Phase 1 |
| `ccplan hook` | Claude CodeのPostToolUse hookとして動作（review + フィードバック） | Phase 2 |
| `ccplan list` | plansDirectory内のプランファイル一覧を表示 | 将来 |
| `ccplan diff` | 2つのプランの差分を表示 | 将来 |
| `ccplan clean` | 古いプランファイルの削除 | 将来 |

### 解決する課題

| 課題 | 現状 | 目指す体験 |
|------|------|-----------|
| 全体像が掴みにくい | プランが長文でターミナルに流れる | 折りたたみ可能なステップ一覧 + サマリービュー |
| 特定箇所を指し示しにくい | 「3番目のやつ」のような曖昧な指示 | ステップを選択してインラインコメント |
| 複数箇所へのフィードバックが面倒 | 1メッセージに全部書く必要がある | 各ステップに個別コメント → まとめて出力 |
| hookスクリプトがbash + jq依存で脆い | bashでJSONLパースは保守性が低い | Go製バイナリで堅牢に処理 |

---

## 技術スタック

- **言語**: Go
- **TUIフレームワーク**: [Bubble Tea](https://github.com/charmbracelet/bubbletea)
- **スタイリング**: [Lip Gloss](https://github.com/charmbracelet/lipgloss)
- **UIコンポーネント**: [Bubbles](https://github.com/charmbracelet/bubbles) — viewport, textarea, key
- **Markdownレンダリング**: [Glamour](https://github.com/charmbracelet/glamour)
- **Markdown AST解析**: [goldmark](https://github.com/yuin/goldmark)（Glamourが内部で使用しているパーサー）
- **CLIフレームワーク**: [kong](https://github.com/alecthomas/kong)

### 自作/既存の判断

TUIの「Markdownビューア」を機能レイヤーで分解し、各レイヤーごとに自作か既存利用かを判断した。

| レイヤー | 何をするか | 判断 | 理由 |
|---------|-----------|------|------|
| Markdown → ANSIレンダリング | MD文字列をスタイル付きターミナルテキストに変換 | **既存: Glamour** | 高品質で安定。自作する理由がない |
| スクロール可能なビューポート | レンダリング済みテキストの表示・スクロール | **既存: bubbles/viewport** | 標準的なUI部品。そのまま使える |
| テキスト入力 | コメント用の複数行テキスト入力 | **既存: bubbles/textarea** | 標準的なUI部品 |
| 見出しベースのステップ分割 | MDを `[]Step` 構造化データに変換 | **自作** | 独自のパースルール（見出しレベル→親子関係、ID採番） |
| ステップ連動ナビゲーション | 左ペインで選択→右ペインの表示を切替 | **自作** | この対話モデルは本ツール固有 |
| インラインコメント付与 | ステップごとにレビューコメントを紐付け | **自作** | コア機能。既存ツールにはない |
| 2ペインレイアウト合成 | 左右ペインのレイアウトとフォーカス管理 | **自作** | Lip Glossで組むが、状態管理は自前 |

> **Glowとの関係**: charmbracelet/glow は最も近い既存TUIだが、1ファイル単位のブラウズ＋閲覧が目的であり、
> ファイル内を構造化して対話的に操作する設計ではない。Glowを改造するより、Glowと同じ構成要素
> （Glamour + viewport + list）を直接組み合わせて構築する方が効率的。

---

## プロジェクト構造

```
ccplan/
├── cmd/
│   ├── cli.go               # kong CLI定義（struct tags ベース）
│   ├── review.go            # ccplan review サブコマンドの実行ロジック
│   ├── locate.go            # ccplan locate サブコマンドの実行ロジック
│   └── hook.go              # ccplan hook サブコマンドの実行ロジック (Phase 2)
├── internal/
│   ├── plan/                # プラン関連の共通ロジック（コアドメイン）【自作】
│   │   ├── model.go         # Plan, Step 等のデータモデル
│   │   ├── parser.go        # goldmark AST → []Step 構造化
│   │   ├── parser_test.go
│   │   ├── review.go        # レビュー結果のMarkdownフォーマッタ
│   │   └── review_test.go
│   ├── locate/              # プランファイル特定【自作】
│   │   ├── locator.go       # LocatePlanFile + ParseHookInput
│   │   ├── settings.go      # plansDirectory設定の解決
│   │   ├── settings_test.go
│   │   ├── transcript.go    # JSONL transcriptパーサー
│   │   └── transcript_test.go
│   ├── hook/                # Claude Code hook統合【自作】(Phase 2)
│   │   ├── input.go         # hook stdinのJSONパーサー
│   │   ├── input_test.go
│   │   └── runner.go        # hookのオーケストレーション
│   ├── pane/                # ターミナルペイン操作【自作】(Phase 2)
│   │   ├── spawner.go       # PaneSpawner interface
│   │   ├── wezterm.go       # WezTerm実装
│   │   ├── tmux.go          # tmux実装（将来）
│   │   └── detect.go        # マルチプレクサ自動検出
│   └── tui/                 # TUI（Bubble Tea + 自作ロジック）
│       ├── app.go           # メインモデル（2ペイン合成 + 状態管理）【自作】
│       ├── app_test.go      # TUI回帰テスト（画面はみ出し・スクロール等）
│       ├── steplist.go      # 左ペイン：ステップ一覧ツリー【自作、bubbles/list参考】
│       ├── detail.go        # 右ペイン：Glamourレンダリング + bubbles/viewport
│       ├── comment.go       # コメント入力（bubbles/textarea をラップ）
│       ├── styles.go        # Lip Gloss スタイル定義
│       └── keymap.go        # キーバインド定義（bubbles/key）
├── main.go
├── go.mod
└── go.sum
```

### TUIコンポーネントと既存ライブラリの対応

```
┌─ steplist.go ────────────────┬─ detail.go ──────────────────────────┐
│                              │                                      │
│  自作ツリーウィジェット        │  bubbles/viewport                    │
│  (bubbles/list の Delegate   │    └─ Glamour でレンダリングした       │
│   パターンを参考に実装)       │       ステップ本文を表示              │
│                              │                                      │
│  入力: internal/plan の       │  comment.go                          │
│        []Step モデル          │    └─ bubbles/textarea をラップ       │
│                              │                                      │
├──────────────────────────────┴──────────────────────────────────────┤
│  app.go: Lip Gloss の JoinHorizontal でペイン合成                    │
│  keymap.go: bubbles/key でキーバインド定義                           │
└────────────────────────────────────────────────────────────────────┘
```

### kong CLI定義

`cmd/cli.go`（Phase 1 時点）:

```go
type CLI struct {
    Review  ReviewCmd  `cmd:"" help:"Review a plan file in TUI"`
    Locate  LocateCmd  `cmd:"" help:"Locate plan file path from transcript"`
    Hook    HookCmd    `cmd:"" help:"Run as Claude Code PostToolUse hook"`
    Version VersionCmd `cmd:"" help:"Show version"`
}

type ReviewCmd struct {
    PlanFile   string `arg:"" help:"Path to the Markdown plan file"`
    Output     string `enum:"clipboard,stdout,file" default:"clipboard" help:"Output method"`
    OutputPath string `help:"File path for file output" type:"path"`
    StatusPath string `help:"File path to write exit status" type:"path"`
    Theme      string `enum:"dark,light" default:"dark" help:"Color theme"`
    NoColor    bool   `help:"Disable colors"`
    // Phase 2 で追加予定:
    // SpawnPane  bool   `help:"Launch TUI in a new terminal pane"`
    // Spawner    string `enum:"wezterm,tmux,auto" default:"auto" help:"Force specific multiplexer"`
    // PaneTarget string `help:"Target pane for pane output (e.g., tmux:.1)"`
}

type LocateCmd struct {
    Transcript string `help:"Path to transcript JSONL file" type:"existingfile"`
    CWD        string `help:"Working directory for resolving relative plansDirectory" default:"." type:"existingdir"`
    Stdin      bool   `help:"Read hook JSON input from stdin"`
    All        bool   `help:"Output all plan files found in transcript"`
    Quiet      bool   `help:"Exit code only (0=found, 1=not found), no output"`
}

// Phase 2
type HookCmd struct {
    Spawner string `enum:"wezterm,tmux,auto" default:"auto" help:"Force specific multiplexer"`
    Theme   string `enum:"dark,light" default:"dark" help:"Color theme"`
    NoColor bool   `help:"Disable colors"`
}
```

`main.go`:

```go
func main() {
    var cli cmd.CLI
    ctx := kong.Parse(&cli,
        kong.Name("ccplan"),
        kong.Description("Claude Code Plan CLI tool"),
        kong.UsageOnError(),
    )
    err := ctx.Run()
    ctx.FatalIfErrorf(err)
}
```

### パッケージ間の依存関係

```
cmd/review  → internal/plan, internal/tui
cmd/locate  → internal/locate
cmd/hook    → internal/locate, internal/hook, internal/pane, internal/plan, internal/tui
                ↑ hook は locate + review の合成

internal/tui    → internal/plan （パース結果のモデルを表示）
internal/hook   → internal/locate （plansDirectory解決のみ）
internal/locate → internal/plan は参照しない（パスの特定のみ）
internal/pane   → 他パッケージへの依存なし
```

`internal/locate` と `internal/plan` を分離しているのがポイント。`locate` はファイルパスの特定だけを責務とし、Markdownの中身には関与しない。これにより `ccplan locate` は純粋なパス解決ツールとして他用途にも使える。

---

## サブコマンド詳細

### `ccplan review` — プランのTUIレビュー

プランファイルをTUIで表示し、各ステップにレビューコメントを付与する。

```
ccplan review [flags] <plan-file>

Arguments:
  plan-file    Path to the Markdown plan file

Flags:
  --output       Output method: clipboard (default), stdout, file
  --output-path  File path for file output
  --status-path  File path to write exit status (submitted / approved / cancelled)
  --theme        Color theme: dark (default), light
  --no-color     Disable colors

Phase 2 で追加予定:
  --output pane  Output method に pane を追加
  --spawn-pane   Launch TUI in a new terminal pane (auto-detects multiplexer)
  --spawner      Force specific multiplexer: wezterm, tmux, auto (default: auto)
  --pane-target  Target pane for pane output (e.g., tmux:.1)
```

#### 使用例

```bash
# 基本（手動起動、クリップボードにコピー）
ccplan review ~/.claude/plans/my-plan.md

# レビュー結果をファイルに出力
ccplan review --output file --output-path ./review.md plan.md

# パイプで別コマンドに渡す
ccplan review --output stdout plan.md | pbcopy
```

---

### `ccplan locate` — プランファイルの特定

transcript JSONLとsettings.jsonからプランファイルのパスを機械的に特定する。
標準出力にパスを出力するだけのシンプルなコマンド。他ツールやスクリプトとの連携に使える。

```
ccplan locate [flags]

Flags:
  --transcript   Path to transcript JSONL file (required unless --stdin)
  --cwd          Working directory for resolving relative plansDirectory (default: .)
  --stdin        Read hook JSON input from stdin (transcript_path / cwd を自動抽出)
  --all          Output all plan files found in transcript (default: latest only)
  --quiet        Exit code only (0=found, 1=not found), no output
```

#### 使用例

```bash
# transcriptファイルを指定してプランファイルを特定
ccplan locate --transcript ~/.claude/projects/.../session.jsonl
# → /home/user/.claude/plans/jaunty-petting-nebula.md

# hook入力をstdinから読んで特定（hookスクリプトとの連携）
echo "$HOOK_INPUT" | ccplan locate --stdin
# → /home/user/.claude/plans/jaunty-petting-nebula.md

# セッション内の全プランファイルを列挙
ccplan locate --transcript session.jsonl --all
# → /home/user/.claude/plans/plan-a.md
# → /home/user/.claude/plans/plan-b.md

# 他コマンドとの組み合わせ
ccplan review "$(ccplan locate --transcript session.jsonl)"
```

#### 検出アルゴリズム

```
1. plansDirectory の解決
   a. {cwd}/.claude/settings.local.json の "plansDirectory" を確認
   b. {cwd}/.claude/settings.json の "plansDirectory" を確認
   c. ~/.claude/settings.json の "plansDirectory" を確認
   d. いずれも未設定ならデフォルト: ~/.claude/plans/
   e. 相対パスの場合は cwd からの相対として解決

2. transcript JSONL のパース（後方から走査）
   a. transcript ファイルを末尾から1行ずつ読む
   b. 各行をJSONパースし、以下に合致する行を探す:
      - assistant メッセージの content 配列内に
        type == "tool_use" かつ name == "Write" のエントリがある
      - その input.file_path が plansDirectory 配下のパスである
   c. --all でない場合、最新の1件を出力して終了

3. バリデーション
   a. 特定されたファイルが実際に存在するか確認
   b. 存在しなければスキップ（--quiet なら exit 1）
```

#### 並行セッション時の安全性

| シナリオ | 動作 |
|---------|------|
| セッションAとBが並行でプラン作成 | 各hookは自身の `transcript_path` のみ参照するため、AのhookはAのプランだけを、BのhookはBのプランだけを正確に特定する |
| プランを作成しないセッション | `permission_mode != "plan"` で早期return。またtranscript内にplansDir配下のWriteがないため空出力 |
| `plansDirectory` がプロジェクトごとに異なる | 設定の優先度チェーンで正しいディレクトリを解決 |

#### transcript JSONLの構造（参考）

```jsonl
{"type":"assistant","message":{"role":"assistant","content":[{"type":"tool_use","id":"toolu_xxx","name":"Write","input":{"file_path":"/home/user/.claude/plans/jaunty-petting-nebula.md","content":"# Plan: ..."}}]},"sessionId":"eb5b0174-...","timestamp":"2025-..."}
```

> **注意**: transcriptのJSON構造はClaude Codeのバージョンで変わる可能性がある。
> パース失敗時は常に空出力 + exit 1 にフォールバックし、呼び出し元がハンドリングする。

---

### `ccplan hook` — Claude Code PostToolUse hookとして動作

Claude Codeの PostToolUse (Write|Edit) hook から呼ばれ、プランファイルへの書き込みを検知して
review → フィードバックの全フローを実行する。
**bashスクリプトを完全に置き換える。**

```
ccplan hook [flags]

Flags:
  --spawner      Force specific multiplexer: wezterm, tmux, auto (default: auto)
  --theme        Color theme: dark (default), light
  --no-color     Disable colors
```

stdinからhookのJSON入力を受け取り、内部で以下を実行:

1. `permission_mode == "plan"` かチェック（plan以外は exit 0）
2. `PLAN_REVIEW_SKIP=1` 環境変数チェック（スキップ、exit 0）
3. `tool_input.file_path` が plansDirectory 配下かチェック（配下でなければ exit 0）
4. `internal/pane` で別ペインに `ccplan review` を起動
5. review の結果に基づき:
   - submitted → stderrにレビュー内容を出力、exit 2（Claudeにフィードバック）
   - approved → exit 0
   - cancelled → exit 0

> **Stop hookではなくPostToolUseを使う理由**: Claude Codeのplanモードでは
> Stop hookが発火しないバグがある。PostToolUse (Write|Edit) はplanモードでも
> 正常に動作し、プランファイルへのWrite/Edit直後にレビューを起動できる。
> また、ユーザーがTUIでコメントなしsubmit/cancelを選ぶことでループを制御できるため、
> 無限ループ防止のための特別な仕組みが不要になる。

#### Claude Code設定

`.claude/settings.json`:

```json
{
  "hooks": {
    "PostToolUse": [
      {
        "matcher": "Write|Edit",
        "hooks": [
          {
            "type": "command",
            "command": "ccplan hook",
            "timeout": 600
          }
        ]
      }
    ]
  }
}
```

`matcher: "Write|Edit"` でWrite/Editツールにフィルタリング。
Claudeはプラン作成時にWrite、修正時にEditを使用するため両方が必要。
プランファイル以外のWrite/Editは `isUnderPlansDir` チェックで弾かれる。

#### 内部フロー（擬似コード）

```go
func RunHook(stdin io.Reader) error {
    // 1. hook入力をパース
    input, err := hook.ParseInput(stdin)
    if err != nil {
        return nil // パース失敗は何もしない
    }

    // 2. 早期リターン判定
    if input.PermissionMode != "plan" {
        os.Exit(0)
    }
    if os.Getenv("PLAN_REVIEW_SKIP") == "1" {
        os.Exit(0)
    }

    // 3. tool_input からプランファイルパスを取得
    if input.ToolInput == nil || input.ToolInput.FilePath == "" {
        os.Exit(0)
    }
    planFile := input.ToolInput.FilePath

    // 4. plansDirectory 配下かチェック
    plansDir := locate.ResolvePlansDir(input.CWD)
    if !isUnderPlansDir(planFile, plansDir) {
        os.Exit(0)
    }

    // 5. 一時ファイルを用意
    reviewOutput := tmpFile("ccplan-review-*.md")
    statusOutput := tmpFile("ccplan-status-*")
    defer cleanup(reviewOutput, statusOutput)

    // 6. 別ペインで review を起動（internal/pane）
    spawner := pane.AutoDetect() // WezTerm → tmux → direct
    err = spawner.SpawnAndWait("ccplan", []string{
        "review",
        "--output", "file",
        "--output-path", reviewOutput,
        "--status-path", statusOutput,
        planFile,
    })
    if err != nil {
        os.Exit(0) // ペイン起動失敗は何もしない
    }

    // 7. 結果判定
    status := readFile(statusOutput)
    switch status {
    case "submitted":
        review := readFile(reviewOutput)
        if review != "" {
            fmt.Fprint(os.Stderr, review)
            os.Exit(2) // Claudeにフィードバック
        }
    }
    os.Exit(0)
}
```

#### hook入力のJSON構造

```json
{
  "session_id": "eb5b0174-0555-4601-804e-672d68069c89",
  "transcript_path": "/home/user/.claude/projects/.../eb5b0174-....jsonl",
  "cwd": "/home/user/projects/myapp",
  "hook_event_name": "PostToolUse",
  "permission_mode": "plan",
  "tool_name": "Write",
  "tool_input": {
    "file_path": "/home/user/.claude/plans/jaunty-petting-nebula.md"
  }
}
```

#### ループ制御

PostToolUseでは無限ループ防止の特別な仕組みは不要。理由:

1. **ユーザー制御**: TUIでコメントなしsubmit/cancel（exit 0）を選べばループは停止する。
   submit（exit 2）を選んだ場合のみClaudeがプランを修正し、再度Write/Editで発火する。
2. **環境変数**: `PLAN_REVIEW_SKIP=1` でhookを一時的に無効化可能。

---

## `ccplan review` TUI 詳細

### 入力: プランファイルのパース

- Markdownファイルをファイルパス引数で受け取る
- Claude Codeが生成する自由形式のMarkdown
- **ステップ分割の基準**: Markdownの見出し（`#`, `##`, `###` 等）で分割

#### パース方式: goldmark AST walk

正規表現による行単位の分割ではなく、goldmarkでMarkdownをASTにパースし、
Heading ノードを起点にツリーを走査してステップを構築する。

- Glamour が内部で goldmark を使っているため、依存が増えない
- コードブロック内の `#` を見出しと誤認しない（ASTでは CodeBlock ノードになる）
- 将来的にリスト項目やリンクの抽出にも拡張しやすい

```go
// 概念的な処理フロー
func Parse(source []byte) (*Plan, error) {
    md := goldmark.New()
    doc := md.Parser().Parse(text.NewReader(source))

    var steps []*Step
    ast.Walk(doc, func(n ast.Node, entering bool) (ast.WalkStatus, error) {
        if heading, ok := n.(*ast.Heading); ok && entering {
            title := extractText(heading, source)
            body := extractBodyUntilNextHeading(heading, source)
            steps = append(steps, &Step{
                Level: heading.Level,
                Title: title,
                Body:  body,
            })
        }
        return ast.WalkContinue, nil
    })

    return buildHierarchy(steps), nil  // level に基づいて親子関係を構築
}
```

#### パースルール

```
# プランタイトル           → プラン全体のタイトル
（見出し前のテキスト）      → preamble（概要セクション）

## ステップ1: 認証基盤     → Step (level 2)
本文...

### 1.1 JWT検証            → Sub-step (level 3, parent = Step 1)
本文...

### 1.2 ミドルウェア登録    → Sub-step (level 3, parent = Step 1)
本文...

## ステップ2: ルーティング  → Step (level 2)
本文...
```

#### パースで抽出する情報

| フィールド | 抽出方法 |
|-----------|---------|
| ID | 見出しの出現順に自動採番（S1, S1.1, S1.2, S2...） |
| タイトル | 見出しテキスト |
| 本文 | 次の見出しまでのMarkdownテキスト |
| 対象ファイル | 本文中のファイルパスパターン（`path/to/file.go` 等）を抽出 |
| 依存関係 | 本文中の他ステップへの言及をベストエフォートで抽出 |

### TUI画面構成

#### メインビュー（2ペインレイアウト）

**ペイン比率**: 左 30% / 右 70%（ターミナル幅に対する比率）。
ターミナル幅が 80 未満の場合はシングルペインモード（左ペインのみ表示、Enterで右に切替）。

```
┌─ Steps ──────────────────────┬─ Detail ─────────────────────────────┐
│                              │                                      │
│  📋 Plan: Auth System        │ ## S2: JWT検証ロジック                │
│                              │                                      │
│  ▶ S1: 認証ミドルウェア [*]  │ pkg/auth/jwt.go にRS256ベースの       │
│    S1.1: JWT検証       [*]   │ JWT検証ロジックを実装する。            │
│    S1.2: MW登録              │                                      │
│  ▶ S2: ルーティング更新      │ 対象ファイル:                         │
│    S2.1: エンドポイント追加  │   pkg/auth/jwt.go                    │
│    S2.2: バリデーション      │   pkg/auth/jwt_test.go               │
│  ▶ S3: テスト追加            │                                      │
│                              │ 依存: S1                             │
│                              │                                      │
│                              │ ─── Review Comment ──────────────── │
│                              │ │ HS256に変更。鍵はenvから読む。    │ │
│                              │ │                                  │ │
│                              │ ────────────────────────────────── │
│                              │                                      │
├──────────────────────────────┴──────────────────────────────────────┤
│ [j/k] navigate  [enter] toggle  [c] comment  [v] viewed  [q] quit │
│ [/] search  [s] submit review  [tab] switch pane  [?] help        │
└────────────────────────────────────────────────────────────────────┘
```

#### 左ペイン：ステップ一覧

- 最上部にプランタイトル（Plan.Title）を表示
- タイトル直下に "Overview" エントリ（Plan.Preamble に対応。選択すると右ペインにpreamble表示）
- その下にツリー形式でステップ群を表示（見出しレベルに基づくインデント）
- 折りたたみ / 展開（Enter or ← →）
- 各ステップのバッジ：`[*]` コメントあり、`[✓]` viewed済み
- 依存関係のインジケータ（例: `S2 ← S1`）（Phase 3）
- カーソル移動: `j`/`k` or ↑/↓
- 実装: bubbles/list の Delegate パターンを参考に自作。bubbles/list そのものは
  フラットリスト向けのため、ツリーの折りたたみ/展開を表現するにはカスタム実装が必要。

#### 右ペイン：ステップ詳細

- 選択中ステップの本文を Glamour でANSIスタイル付きテキストに変換し、bubbles/viewport で表示
- Glamour の dark テーマをベースにカスタムスタイルを適用（Chroma error トークンの赤背景を除去。
  CJK テキストが error トークンとして誤認識されると赤背景になるため）
- 対象ファイル一覧（ハイライト表示）（Phase 3）
- 依存関係の表示（Phase 3）
- レビューコメント入力エリア（下部、bubbles/textarea をラップした comment ウィジェット）

#### コメント入力モード

- `c` キーで開始、textarea ウィジェットで複数行入力
- `Esc` でキャンセル、`Ctrl+S` で確定
- `Tab` でConventional Commentsラベルをサイクル切替
- ラベル: `suggestion` (デフォルト), `issue`, `question`, `nitpick`, `todo`, `thought`, `note`, `praise`, `chore`

### 出力

#### レビュー結果フォーマット

```markdown
# Plan Review

以下のレビューコメントに基づいてプランを修正してください。

## S1.1: JWT検証 [suggestion]
HS256に変更。鍵は環境変数 `JWT_SECRET` から読み込む方式にして。

## S2: ルーティング更新 [issue]
既存のルーティング実装で対応可能なため、このステップは不要。

## S3: テスト追加 [question]
カバレッジ目標は80%と90%のどちら？
```

> フォーマット: `## {StepID}: {StepTitle} [{ActionType}]`
> ステップタイトルを含めることで、Claudeがどのステップへのフィードバックか正確に把握できる。

#### 出力先

| 方法 | 説明 | フラグ |
|------|------|--------|
| **クリップボード** (デフォルト) | クリップボードにコピー | `--output clipboard` |
| **標準出力** | パイプ連携用 | `--output stdout` |
| **ファイル** | 指定パスに書き出し | `--output file --output-path ./review.md` |
| **ペイン送信** (将来) | 別ペインに送信 | `--output pane` |

#### ステータスファイル（hook連携用）

`--status-path` 指定時、TUI終了時に結果ステータスを書き出す:

| ステータス | 意味 | hookの動作 |
|-----------|------|-----------|
| `submitted` | レビューをsubmitした | レビューファイルを読んで exit 2 |
| `approved` | コメントなし | exit 0（続行） |
| `cancelled` | `q` で中断した | exit 0（何もしない） |

### キーバインド一覧

| キー | コンテキスト | アクション |
|------|-------------|-----------|
| `j` / `↓` | ステップ一覧 | 次のステップに移動 |
| `k` / `↑` | ステップ一覧 | 前のステップに移動 |
| `Enter` | ステップ一覧 | 折りたたみ/展開をトグル |
| `→` / `l` | ステップ一覧 | 展開 |
| `←` / `h` | ステップ一覧 | 折りたたみ |
| `Tab` | 全体 | 左ペイン ↔ 右ペインのフォーカス切り替え |
| `c` | ステップ選択中 | コメント入力モードを開始 |
| `v` | ステップ選択中 | viewedマークをトグル |
| `/` | ステップ一覧 | 検索モードを開始 |
| `Tab` | コメント入力中 | ラベルをサイクル切替 |
| `Ctrl+S` | コメント入力中 | コメントを確定 |
| `Esc` | コメント入力中 / 検索中 | キャンセル |
| `Enter` | 検索中 | 検索を確定 |
| `s` | 全体 | レビューをsubmit（出力して終了） |
| `q` / `Ctrl+C` | 全体 | 終了（未保存コメントがあれば確認） |
| `?` | 全体 | ヘルプ表示 |

---

## Claude Code統合 — PostToolUseフックによるシームレス起動

### PostToolUse (Write|Edit) hookを使う理由

| hookイベント | 検討結果 |
|-------------|---------|
| Stop | planモードでは発火しないバグがある（Claude Codeの承認待ち状態がStopとみなされない）。 |
| **PostToolUse (Write\|Edit)** ✅ | planモードでも正常に発火。`tool_input.file_path` でプランファイルを直接判定可。exit 2でフィードバック注入。ユーザーがTUIでコメントなしsubmit/cancelを選ぶことでループを制御。Claudeはプラン作成時にWrite、修正時にEditを使用。 |
| Notification | フィードバックをClaudeに戻す仕組みがない。 |

### 動作フロー

```
┌─────────────────────────────────────────────────────────────────┐
│ Claude Code (plan mode)                                         │
│                                                                 │
│  1. ユーザーが計画を依頼                                         │
│  2. Claudeがコードベースを分析してプランを生成                    │
│  3. Claudeがプランファイルを Write/Edit → PostToolUse hook発火     │
│  4. ccplan hook が stdin から hook入力を受け取る                  │
│                                                                 │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │ ccplan hook (Go)                                         │   │
│  │                                                          │   │
│  │  5. permission_mode == "plan" を確認                      │   │
│  │  6. PLAN_REVIEW_SKIP チェック                              │   │
│  │  7. tool_input.file_path が plansDir 配下か判定            │   │
│  │  8. internal/pane で隣接ペインに ccplan review を起動       │   │
│  │  9. review プロセスの完了を待機                             │   │
│  │                                                          │   │
│  │  ┌──────────────────────────────────────────────────┐    │   │
│  │  │ ccplan review (別ペイン)                         │    │   │
│  │  │                                                  │    │   │
│  │  │  ユーザーがプランをレビュー                        │    │   │
│  │  │  各ステップにコメント付与                          │    │   │
│  │  │  "s" でsubmit → レビュー結果をファイルに出力       │    │   │
│  │  │  TUI終了 → ペインが閉じる                         │    │   │
│  │  └──────────────────────────────────────────────────┘    │   │
│  │                                                          │   │
│  │  10. 結果判定:                                            │   │
│  │      submitted → stderr にレビュー内容、exit 2            │   │
│  │      approved  → exit 0                                   │   │
│  │      cancelled → exit 0                                   │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                 │
│  11-a. exit 2: Claude がレビューを受け取りプラン修正             │
│        → 再度Write/Edit → PostToolUse発火 → ユーザーが再レビュー可 │
│  11-b. exit 0: Claude が続行、ユーザーが判断                     │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## ターミナルマルチプレクサ抽象化

### インタフェース

```go
// PaneSpawner はターミナルマルチプレクサの抽象化
type PaneSpawner interface {
    // SpawnAndWait は新しいペインでコマンドを実行し、完了を待つ
    SpawnAndWait(cmd string, args []string) error
    // Available はこのスポーナーが利用可能かを返す
    Available() bool
    // Name はマルチプレクサ名を返す
    Name() string
}
```

### WezTerm実装

```go
func (w *WezTermSpawner) SpawnAndWait(cmd string, args []string) error {
    // 1. ペイン分割して起動
    fullCmd := append([]string{cmd}, args...)
    splitArgs := append([]string{
        "cli", "split-pane", "--bottom", "--percent", "80", "--",
    }, fullCmd...)
    out, err := exec.Command("wezterm", splitArgs...).Output()
    if err != nil {
        return err
    }
    paneID := strings.TrimSpace(string(out))

    // 2. ペインの終了をポーリングで待機
    for {
        listOut, _ := exec.Command("wezterm", "cli", "list", "--format", "json").Output()
        if !paneExists(listOut, paneID) {
            break
        }
        time.Sleep(500 * time.Millisecond)
    }
    return nil
}
```

### 自動検出

```go
func AutoDetect() PaneSpawner {
    spawners := []PaneSpawner{
        &WezTermSpawner{},
        &TmuxSpawner{},   // 将来
    }
    for _, s := range spawners {
        if s.Available() {
            return s
        }
    }
    return &DirectSpawner{} // フォールバック: 現在のターミナルで直接起動
}
```

---

## ペイン送信機能（将来拡張）

`ccplan review --output pane` で、レビュー結果をClaude Codeが動いている別ペインに直接送信。

| 環境 | 送信方法 |
|------|---------|
| tmux | `tmux send-keys -t {target-pane} "テキスト" Enter` |
| WezTerm | `wezterm cli send-text --pane-id {id} "テキスト"` |

- `--pane-target` で明示指定、未指定なら環境変数から自動検出
- 長文時はtmpファイル経由 + `@` 参照コマンドとして送信

---

## データモデル

### internal/plan/model.go

```go
// Plan はパース済みプランの全体構造
type Plan struct {
    Title    string  // H1見出し（なければファイル名）
    Preamble string  // 最初の見出し前のテキスト（概要）
    Steps    []*Step // トップレベルのステップ群
}

// Step はプラン内の1ステップ（見出し1つに対応）
type Step struct {
    ID       string  // 自動採番 "S1", "S1.1", "S2" 等
    Title    string  // 見出しテキスト（"## " 部分は除去済み）
    Level    int     // 見出しレベル（2=##, 3=###, ...）
    Body     string  // 見出しから次の見出しまでのMarkdown原文
    Children []*Step // 下位レベルの子ステップ
    Parent   *Step   // 親ステップ（nil ならトップレベル）

    // パース時にベストエフォートで抽出（Phase 3）
    FilePaths    []string // 本文中に出現するファイルパス
    Dependencies []string // 他ステップへの参照（"S1" 等）
}

// StepID は見出しの出現順とレベルから決定する。
// H1 は Plan.Title に使うため、ステップのIDはH2以降が対象。
//
//   ## Foo      → S1
//   ### Bar     → S1.1
//   ### Baz     → S1.2
//   ## Qux      → S2
//   ### Quux    → S2.1
//   #### Deep   → S2.1.1
```

### internal/plan/review.go

```go
// ReviewComment は1ステップへのレビューコメント
type ReviewComment struct {
    StepID string     // 対象ステップのID
    Action ActionType // コメントのアクション種別
    Body   string     // コメント本文
}

// ActionType はレビューコメントのアクション種別
type ActionType string

const (
    ActionSuggestion ActionType = "suggestion" // デフォルト
    ActionIssue      ActionType = "issue"
    ActionQuestion   ActionType = "question"
    ActionNitpick    ActionType = "nitpick"
    ActionTodo       ActionType = "todo"
    ActionThought    ActionType = "thought"
    ActionNote       ActionType = "note"
    ActionPraise     ActionType = "praise"
    ActionChore      ActionType = "chore"
)

// ReviewResult はレビュー全体の結果
type ReviewResult struct {
    Comments []ReviewComment
    Status   Status
}

// Status はTUI終了時のステータス
type Status string

const (
    StatusSubmitted Status = "submitted" // コメントあり
    StatusApproved  Status = "approved"  // コメントなし
    StatusCancelled Status = "cancelled" // q で中断
)
```

### ステータス判定ルール

| ユーザー操作 | Status | 理由 |
|-------------|--------|------|
| `s` を押してsubmit、コメントが1つ以上ある | `submitted` | フィードバックがある |
| `s` を押してsubmit、コメントが0件 | `approved` | 修正不要の意思表示 |
| `q` / `Ctrl+C` で中断 | `cancelled` | レビュー中断 |

---

## TUI状態遷移

### アプリケーションモード

```go
type AppMode int

const (
    ModeNormal   AppMode = iota // ステップ一覧のナビゲーション
    ModeComment                 // コメント入力中（textareaにフォーカス）
    ModeConfirm                 // 確認ダイアログ（未保存コメントがある状態で q を押した等）
    ModeHelp                    // ヘルプオーバーレイ表示中
    ModeSearch                  // ステップ検索（インクリメンタルフィルタ）
)
```

### 状態遷移図

```
                    ┌──────────────┐
                    │  ModeNormal  │◄──────────────────────┐
                    └──────┬───────┘                       │
                           │                               │
             c キー        │         ? キー                │ Esc / Enter
                           ▼                               │
                    ┌──────────────┐                ┌──────┴───────┐
                    │ ModeComment  │                │   ModeHelp   │
                    └──────┬───────┘                └──────────────┘
                           │
           Ctrl+S (確定)   │    Esc (キャンセル)
           ────────────────┤────────────────────► ModeNormal
                           │
                           ▼
                    コメント保存
                    → ModeNormal

     q キー (ModeNormal)
           │
           ├── 未保存コメントなし → 即座に終了
           │
           └── 未保存コメントあり
                    │
                    ▼
              ┌──────────────┐
              │ ModeConfirm  │
              └──────┬───────┘
                     │
         y (終了)    │    n (キャンセル)
         ────────────┤─────────────────► ModeNormal
                     │
                     ▼
                アプリ終了（StatusCancelled）
```

### フォーカス管理

```
ModeNormal:
  フォーカス = 左ペイン（ステップ一覧）
  Tab で右ペイン（viewport）にフォーカス移動可
  右ペインフォーカス時: j/k でスクロール、Tab で左に戻る

ModeComment:
  フォーカス = 右ペイン下部の textarea
  左ペインのキー操作は無効
  Esc でキャンセル、Ctrl+S で確定 → ModeNormal に戻る
```

---

## パーサーのエッジケース処理

| ケース | 処理 |
|-------|------|
| 見出しが1つもない | Plan全体を1つの preamble として扱う。ステップ一覧は空、右ペインにpreamble表示 |
| H1のみ（H2以降なし） | Title = H1テキスト、本文全体がpreamble |
| H1が複数ある | 最初のH1をTitle、2つ目以降のH1もステップとして扱う（Level 1 → S1, S2, ...） |
| H3がH2なしで出現（レベル飛び） | そのままのレベルでステップとして登録。親はなし（トップレベル扱い） |
| コードブロック内の `#` | goldmark ASTでは CodeBlock/FencedCodeBlock ノードになるため見出しと誤認しない |
| 空の見出し `## ` | Title = ""（空文字列）、ID は通常通り採番 |
| 非常に長い本文（1000行超） | パース自体は行う。viewportのスクロールで対応。パフォーマンス問題はPhase 3で対応 |

---

## コメント操作の詳細

### コメントのライフサイクル

| 操作 | 方法 |
|------|------|
| 新規作成 | `c` キーでtextareaを開いて入力、`Tab` でラベル切替、`Ctrl+S` で確定 |
| 編集 | 既にコメントがあるステップで `c` → 既存コメントとラベルがプリフィルされた状態で編集 |
| 削除 | 既にコメントがあるステップで `c` → テキストを全消去して `Ctrl+S` |
| viewed マーク | `v` キー → ステップを確認済みとしてマーク（レビュー出力には含まれない） |

### Conventional Comments ラベル

コメント入力時に `Tab` キーで以下のラベルをサイクル切替:

| ラベル | 用途 |
|--------|------|
| `suggestion` (デフォルト) | 改善提案 |
| `issue` | 具体的な問題の指摘 |
| `question` | 質問・確認 |
| `nitpick` | 些細な好みの問題 |
| `todo` | 必要な小さな変更 |
| `thought` | アイデア・参考情報 |
| `note` | 情報の強調 |
| `praise` | ポジティブな点 |
| `chore` | 受け入れ前に必要な作業 |

---

## クリップボード

- ライブラリ: [atotto/clipboard](https://github.com/atotto/clipboard)
- macOS: `pbcopy` / Linux: `xclip` or `xsel` / WSL: `clip.exe`
- atotto/clipboard がこれらを内部で自動判別するため、明示的な分岐は不要
- クリップボード使用不可の環境（SSH先のサーバ等）では、コピー失敗時にエラーメッセージを表示し、
  `--output stdout` または `--output file` の使用を提案する

---

## エラーハンドリング方針

| コンポーネント | エラー種別 | 対応 |
|--------------|-----------|------|
| `ccplan review` | プランファイルが存在しない | エラーメッセージを出力して exit 1 |
| `ccplan review` | プランファイルのパース失敗 | 可能な範囲でパース。完全に壊れている場合はエラーメッセージ + exit 1 |
| `ccplan review` | ターミナルサイズ取得失敗 | 80x24 をデフォルトとしてフォールバック |
| `ccplan review` | クリップボードコピー失敗 | stderrにエラー表示、exit code は 0（レビュー自体は完了） |
| `ccplan locate` | transcriptファイルが存在しない | エラーメッセージ + exit 1 |
| `ccplan locate` | JSONL パース失敗（一部行） | その行をスキップして続行 |
| `ccplan locate` | プランファイルが見つからない | 空出力 + exit 1 |
| `ccplan locate` | settings.json が壊れている | その設定ファイルをスキップ、次の優先度へフォールバック |
| `ccplan hook` | hook入力JSONのパース失敗 | exit 0（何もしない）。stderrにログ出力 |
| `ccplan hook` | ペインスポーン失敗 | DirectSpawnerにフォールバック。それも失敗なら exit 0 |
| `ccplan hook` | reviewプロセスが異常終了 | exit 0（何もしない） |

**原則**: `ccplan hook` は決してClaude Codeのワークフローを壊さない。
あらゆる想定外のエラーで exit 0（何もしない）にフォールバックする。

---

## テスト方針

### ユニットテスト

| パッケージ | テスト対象 | テスト方法 |
|-----------|-----------|-----------|
| `internal/plan` | Markdownパーサー | テスト用.mdファイルを `testdata/` に配置。各エッジケースのgoldenファイルテスト |
| `internal/plan` | レビュー結果フォーマッタ | ReviewResult → Markdown文字列の変換を検証 |
| `internal/locate` | transcript JSONLパーサー | テスト用.jsonlファイルを `testdata/` に配置 |
| `internal/locate` | plansDirectory解決 | 一時ディレクトリにsettings.jsonを配置してテスト |
| `internal/hook` | hook入力パーサー（Phase 2） | 各種JSON入力パターン（正常、plan以外、tool_input null等） |

### テストデータの管理

```
ccplan/
├── internal/
│   ├── plan/
│   │   └── testdata/
│   │       ├── basic.md              # 基本的なプラン
│   │       ├── no-headings.md        # 見出しなし
│   │       ├── deep-nesting.md       # H4以降の深いネスト
│   │       ├── code-block-hash.md    # コードブロック内に # がある
│   │       └── single-step.md        # ステップが1つだけ
│   └── locate/
│       └── testdata/
│           ├── transcript-with-plan.jsonl
│           ├── transcript-no-plan.jsonl
│           ├── transcript-multiple-plans.jsonl
│           └── transcript-malformed.jsonl
```

### TUIテスト

`internal/tui/app_test.go` に回帰テストを実装済み:

- **画面はみ出し防止**: 各モード（Normal, Comment, Confirm, Help）× 複数ターミナルサイズで
  `View()` の出力行数がターミナル高さを超えないことを検証
- **スクロール追従**: カーソルを可視範囲外まで移動した際に、選択中のステップが
  レンダリング結果に含まれることを検証

> **実装メモ**: `tea.Program` を介さず、`NewApp()` → `Update(WindowSizeMsg)` → `View()` で
> 直接モデルをテストする方式。E2E的な操作テストは将来 teatest で対応予定。

---

## ビルド・配布

### Go バージョン・モジュール

```
Go: 1.26
Module: github.com/koh-sh/ccplan
```

### ビルド

```bash
# 開発用
go build -o ccplan .

# バージョン埋め込み
go build -ldflags "-X main.version=$(git describe --tags)" -o ccplan .
```

### インストール

```bash
# go install
go install github.com/koh-sh/ccplan@latest

# バイナリを PATH の通った場所に配置
# hookから呼ばれるため、ccplan が PATH 上にある必要がある
```

### CI（GitHub Actions想定）

- `go vet` / `staticcheck`
- `go test ./...`
- goreleaser でクロスコンパイル + リリース（darwin/arm64, darwin/amd64, linux/amd64）

---

## 非機能要件

- **起動速度**: 100ms以内（プランファイルのパースを含む）
- **対応プランサイズ**: 1000行程度のMarkdownまで快適に動作
- **ターミナル互換性**: 256色 + True Color対応。最低限16色でもレイアウト崩れなし
- **最小ターミナルサイズ**: 80x24（これ以下ではシングルペインに自動切替）
- **依存関係**: 外部バイナリへの依存なし（Go単体でビルド）。
  hookモードのペイン起動時のみ `wezterm` or `tmux` CLIが必要（なければフォールバック）。
- **transcript JSONL互換性**: パース失敗時は常にフォールバック動作（exit 0 / 空出力）。
  Claude Codeバージョン変更時の破損を最小化。

---

## 開発フェーズ

### Phase 1: MVP

- **`ccplan review`**: TUIビューア + レビュー機能
  - Markdownパーサー（goldmark ASTから見出しベースのステップ分割）
  - 2ペインTUI（ステップ一覧ツリー + Glamour/viewport による詳細表示）
  - 折りたたみ/展開
  - コメント入力（bubbles/textarea）
  - 出力（clipboard / stdout / file）
  - `--status-path`
- **`ccplan locate`**: transcript解析 + plansDirectory解決
  - JSONL transcriptパーサー
  - settings.json の plansDirectory 解決チェーン
  - `--stdin` / `--transcript` / `--all` フラグ

### Phase 2: Claude Code統合

- **`ccplan hook`**: PostToolUse (Write|Edit) hookオーケストレーション
  - hook入力パーサー（permission_mode / tool_input.file_path）
  - plansDir判定 → review → exit code の合成フロー
- WezTerm PaneSpawner実装（`--spawn-pane`）
- E2Eフローの検証

### Phase 3: 可視化強化

- 依存関係のツリー表示・矢印
- ファイルパスの自動抽出とハイライト（goldmark ASTからコードスパン・リンクを走査）
- Glamourのカスタムスタイル（テーマ対応、ファイルパス専用スタイル等）

### Phase 4: マルチプレクサ対応拡張 + 将来サブコマンド

- tmux PaneSpawner実装
- `ccplan review --output pane`
- `ccplan list`: plansDirectory内のプラン一覧
- `ccplan diff`: プラン間の差分表示
- `ccplan clean`: 古いプランの掃除
